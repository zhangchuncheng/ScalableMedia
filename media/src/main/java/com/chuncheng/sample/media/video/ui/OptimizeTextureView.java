...
/**
 * Description: TextureView实现显示视频内容
 * 1.重写onMeasure方法，实现view的大小随视频的宽高比动态缩放。
 *
 * @author: zhangchuncheng
 * @date: 2017/3/15
 */

public class OptimizeTextureView extends TextureView {
    /** 按照宽度进项缩放 */
    public static final int SCALE_TYPE_WIDTH = 1;
    /** 按照高度进项缩放 */
    public static final int SCALE_TYPE_HEIGHT = 2;
    /** 不指定缩放拉伸填充布局 */
    public static final int SCALE_TYPE_FILL = 3;
    /** 默认缩放模式根据控件的宽高比 */
    ...
    public OptimizeTextureView(Context context) {
        super(context);
        initView();
    }

    public OptimizeTextureView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public OptimizeTextureView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initView();
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public OptimizeTextureView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    public void setVideoSize(Point videoSize) {
        if (videoSize.x > 0 && videoSize.y > 0
                && (videoSize.x != mPoint.x || videoSize.y != mPoint.y)) {
